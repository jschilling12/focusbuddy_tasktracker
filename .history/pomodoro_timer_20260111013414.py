...
class PomodoroTimer:
    ...
    def __init__(self):
        ...
        self.root.tk.call('wm', 'iconphoto', self.root._w, PhotoImage(file=resource_path("pomodoro_icon.png")))
        ...
        self.short_break_timer = ttk.Label(self.tab2, text='05:00', font=('Ubuntu', 48))
        ...
        self.start_button = ttk.Button(self.grid_layout, text="Stop", command=self.end_timer)
    ...
    def start_timer(self):
        ...
        if timer_id == 1:
            ...
            while full_seconds > 0 and not self.stopped:
                minutes, seconds = divmod(full_seconds, 60)
                self.short_break_timer.config(text=f"{minutes:02d}:{seconds:02d}")
                self.root.update()
                time.sleep(1)
                full_seconds -= 1
            if not self.stopped or self.skipped:
